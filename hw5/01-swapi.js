...
const starships = [
    ...
    const result = input.map( 
        (item) => {
            if(item.cost_in_credits !== "unknown"){
                let cost =  parseInt(item.cost_in_credits);
                return `${item.name}, manufactured by ${item.manufacturer} - cost: ${cost.toLocaleString()} credits`
            }
            else{
  ...
  const filterStarships = (input) => {
    ...
    const result = input.filter(
        (item) => {  
            let passengers =  parseInt(item.passengers);   
            let crew = parseInt(item.crew);   
